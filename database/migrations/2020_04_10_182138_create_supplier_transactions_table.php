...
class CreateSupplierTransactionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('supplier_transactions', function (Blueprint $table) {
            $table->id();
            $table->enum('payment_type',['Add Balance', 'Opening Balance', 'Purchase Payment', 'Withdraw', 'Adjustment Balance'])->default('Add Balance')->comment('Adjustment Balance using for extra payment on invoice');
            ...
            $table->foreignId('suppliers_id')->constrained()->onDelete('cascade')->onUpdate('No Action');
